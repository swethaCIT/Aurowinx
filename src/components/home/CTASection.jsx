...
// Requires: framer-motion, three (r128)
...
function GlitchText({ children, style }) {
  ...
  useEffect(() => {
    const id = setInterval(() => { setG(true); setTimeout(() => setG(false), 180); }, 3800 + Math.random() * 2500);
  ...
  return (
    <span style={{ position: "relative", display: "inline-block", ...style }}>
      {children}
      {g && <>
        <span aria-hidden style={{ position: "absolute", inset: 0, color: "#06b6d4", clipPath: "polygon(0 28%,100% 28%,100% 48%,0 48%)", transform: "translateX(-3px)", opacity: 0.7 }}>{children}</span>
        <span aria-hidden style={{ position: "absolute", inset: 0, color: "#8b5cf6", clipPath: "polygon(0 62%,100% 62%,100% 76%,0 76%)", transform: "translateX(3px)", opacity: 0.7 }}>{children}</span>
      </>}
    </span>
  );
}
...
function CountUp({ to, suffix = "", inView }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => { n += step; if (n >= to) { setV(to); clearInterval(id); } else setV(n); }, 22);
    return () => clearInterval(id);
  }, [inView, to]);
  return <>{v}{suffix}</>;
}
...
/* ════════════════════════════════════════════════════════
   HOLO CARD  — better images
════════════════════════════════════════════════════════ */
const IMAGES = [
  {
    src: "https://images.unsplash.com/photo-1601132359864-c974e79890ac?w=800&q=85",
    alt: "Silicon chip close-up", label: "ASIC Design", span: "wide",
  },
  {
    src: "https://images.unsplash.com/photo-1635070041078-e363dbe005cb?w=800&q=85",
    alt: "Semiconductor wafer", label: "Silicon Fab",
  },
  {
    src: "https://images.unsplash.com/photo-1597852074816-d933c7d2b988?w=800&q=85",
    alt: "PCB electronics engineering", label: "PCB & Firmware",
  },
  {
    src: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&q=85",
    alt: "IoT connected devices", label: "IoT & Embedded",
  },
];
    ...
    >
      ...
      >
        ...
        <div style={{ position: "absolute", bottom: 12, left: 12, display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ color: "#fff", fontSize: 10, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase" }}>{label}</span>
        </div>
      </motion.div>
    </motion.div>
  );
...
function HUDBracket({ position }) {
  ...
  return <div style={s}><svg width="22" height="22" viewBox="0 0 22 22" fill="none">
    <path d={isTop && isLeft ? "M0 14L0 0L14 0" : isTop ? "M8 0L22 0L22 14" : isLeft ? "M0 8L0 22L14 22" : "M8 22L22 22L22 8"}
  ...
  return (
    <>
      ...
      <section ref={ref} className="cta-wrap" style={{
          ...
          <div className="hero-grid">
            ...
            <motion.div>
              ...
              >
                ...
                <span style={{ color: "#0891b2", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase" }}>
              ...
              {/* CTAs */}
              <motion.div className="btn-row" initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}}
              ...
              {/* Stats */}
              <motion.div className="stats-row" initial={{ opacity: 0, y: 18 }} animate={inView ? { opacity: 1, y: 0 } : {}}
                transition={{ duration: 0.65, delay: 0.48 }}
                style={{ marginTop: 44, paddingTop: 26, borderTop: "1px solid #e2e8f0" }}>
                {STATS.map(s => (
                  <div key={s.label}>
                    <p style={{ fontSize: 27, fontWeight: 800, color: "#0a0f2c", margin: "0 0 2px", letterSpacing: "-0.04em", lineHeight: 1 }}>
                      <CountUp to={s.num} suffix={s.suffix} inView={inView} />
                    </p>
                    <p style={{ fontSize: 10, color: "#94a3b8", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.14em", margin: 0 }}>
                      {s.label}
                    </p>
                  </div>
                ))}
            ...
            {/* RIGHT — 3D chip canvas */}
            <motion.div className="canvas-wrap"
              initial={{ opacity: 0, scale: 0.9 }} animate={inView ? { opacity: 1, scale: 1 } : {}}
              transition={{ duration: 1, delay: 0.12, ease: [0.22, 1, 0.36, 1] }}>
              {/* Light-blue-to-white backdrop */}
              ...
              <motion.div animate={{ y: [0,-6,0] }} transition={{ duration: 3.5, repeat: Infinity, ease: "easeInOut" }}
                ...
                <p style={{ color: "#0891b2", fontSize: 9, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", margin: "0 0 2px" }}>System Online</p>
                <p style={{ color: "#0f172a", fontSize: 13, fontWeight: 700, margin: 0, fontFamily: "'JetBrains Mono',monospace" }}>AurowinX Core v2.4</p>
              </motion.div>
              <motion.div animate={{ y: [0,6,0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut", delay: 1 }}
                ...
                <p style={{ color: "#7c3aed", fontSize: 9, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", margin: "0 0 2px" }}>Response</p>
                <p style={{ color: "#0f172a", fontSize: 13, fontWeight: 700, margin: 0, fontFamily: "'JetBrains Mono',monospace" }}>&lt; 24 hrs</p>
              </motion.div>
            </motion.div>
        ...
        <motion.div style={{ y: parallaxY }}>
          <div style={{ maxWidth: 1280, margin: "0 auto" }} className="img-pad">
            <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ delay: 0.28 }}
              ...
              <span style={{ color: "#94a3b8", fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase" }}>Our Engineering Domains</span>
              <div style={{ flex: 1, height: "0.5px", background: "#e2e8f0" }} />
            </motion.div>
        ...
        {/* ══════════════════════════════════════════
            FOOTER — dark navy with rich content
        ══════════════════════════════════════════ */}
        <footer style={{ background: "#060d1f", position: "relative", overflow: "hidden", marginTop: 8 }}>
          ...
          <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative", zIndex: 2 }} className="footer-top">
            ...
            {/* Services */}
            <div>
              <p style={{ color: "rgba(255,255,255,0.55)", fontSize: 10, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", margin: "0 0 18px" }}>Services</p>
            ...
            {/* Company */}
            <div>
              <p style={{ color: "rgba(255,255,255,0.55)", fontSize: 10, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", margin: "0 0 18px" }}>Company</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
                {FOOTER_COMPANY.map(s => (
                  <a key={s} href="#"
                    ...
                    onMouseLeave={e => { e.currentTarget.style.color = "rgba(255,255,255,0.45)"; }}
                  >
                    <span style={{ width: 4, height: 4, borderRadius: "50%", background: "#8b5cf6", display: "inline-block", flexShrink: 0 }} />
                    {s}
                  </a>
                ))}
              </div>
            ...
            {/* Newsletter / CTA */}
            <div>
              <p style={{ color: "rgba(255,255,255,0.55)", fontSize: 10, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", margin: "0 0 18px" }}>Stay Updated</p>
          ...
          <div style={{ maxWidth: 1280, margin: "0 auto" }} className="footer-bottom">
            ...
            <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
              {["Privacy Policy", "Terms of Use", "Sitemap"].map(l => (
                <a key={l} href="#" style={{ color: "rgba(255,255,255,0.2)", fontSize: 12, textDecoration: "none", transition: "color .2s" }}
